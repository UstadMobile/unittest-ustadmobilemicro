...
controlServer.httpProc = http.createServer(function (req, res) {
  ...
  if (req.method === 'POST') {
	    ...
	    if (device.lastIndexOf("lg", 0) === 0){
		device = "lg";
	    }else if(device.lastIndexOf("nokia", 0) === 0){
		device="nokia";
	    }else if(device.lastIndexOf("samsung", 0) === 0){
	   	device="samsung";
	    }else if(device.lastIndexOf("alcatel", 0) === 0){
		device="alcatel";
	    }else if(device.lastIndexOf("apple", 0) === 0){
		device="apple";
	    }else{
	 	device="unknown"
	    }
	    console.log("The device is: " + device);
            ...
            var testResultsFilename = path.join(controlServer.resultDumpDir,
                "node-qunit-testresults-" +device+".txt");
